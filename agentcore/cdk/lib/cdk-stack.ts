...
export class AgentCoreStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: AgentCoreStackProps) {
    ...
    // S3 Guard tool permissions. The L3 construct already grants the runtime
    // role bedrock:InvokeModel*; the agent's own tools get exactly the S3
    // calls they make and nothing else. The single write action is scoped to
    // the demo bucket prefix so even an approved write cannot touch other
    // buckets. IAM is the outer blast-radius limit; the approve_writes gate
    // in main.py is the inner one.
    for (const env of this.application.environments.values()) {
      ...
      env.runtime.role.addToPrincipalPolicy(
        new iam.PolicyStatement({
          sid: 'S3GuardRemediate',
          actions: ['s3:PutBucketPublicAccessBlock'],
          resources: ['arn:aws:s3:::s3guard-demo-*'],
        })
      );
    }
